...
exports.run = (bot, message, args, tools) => {
    ...
    if (args.length == 0) {
        ...
        if (rpg[message.author.id].skill_points <= 0) {
            ...
            rpg[message.author.id].max_hp++;
            rpg[message.author.id].hp++;
            ...
            .addField("Max HP", `${parseInt(rpg[message.author.id].max_hp)-1} => ${rpg[message.author.id].max_hp}`, true)
            .addField("Remaining HP", `${parseInt(rpg[message.author.id].hp)-1} => ${rpg[message.author.id].hp}`, true)
